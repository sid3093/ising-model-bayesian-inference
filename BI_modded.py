...
@njit
def simulate(N, T, J, steps=2500, equil_steps=2000, seed=0):
    np.random.seed(seed) # Numba strictly requires an integer seed
...
if __name__ == "__main__":
    ...
    data = np.loadtxt("CrI3_FC_Bulk.csv", delimiter=",", skiprows=1)
    T = data[:, 0]
    M_raw = data[:, 1]
